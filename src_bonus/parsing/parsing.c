...
/*   Updated: 2024/01/08 14:15:10 by surkim           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_switch(t_data *data, char **strstr)
{
	...
	else if (ft_strncmp(strstr[0], "hb", 3) == 0)
		parsing_hyperboloid(data, strstr);
	else if (ft_strncmp(strstr[0], "NM", 3) == 0)
		parsing_normalmap(data, strstr);
	else
...
void	check(t_data *data)
{
	...
	if ((data->status >> 1) % 2 == 0)
		print_error("파일요소가 올바르지 않습니다.");
	if ((data->status % 2 == 0) && data->light == 0)
		print_error("파일요소가 올바르지 않습니다.");
}
